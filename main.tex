...
Partial Least Squares (PLS) prioritize relevance to both X (predictor) and y (response) variables, rather than maximising the variance in X alone (Figure 2.3) [27]. By combining the PCA with linear regression, PLSR constructs 
...
predict ragweed pollen concentrations, with RF delivering the most accurate predictions [30,31]. A graphical representation of the RF model with decision trees can be found in the figure (Figure 2.4) . \\
...
\textbf{Pseudocode}:
Function read_summarizedexperiment_asd(path):
    Input: path to a single .asd file or directory containing multiple .asd files
    Output: SummarizedExperiment object
    
    Step 1: Check if the path is a file or directory
    Step 2: If directory, list all .asd files in the directory
    Step 3: For each .asd file:
        a. Use `read.asd` to extract spectral data
        b. Use `extract.metadata` to obtain sample metadata
    Step 4: Combine spectral data and metadata into matrices
    Step 5: Create SummarizedExperiment object with:
        - Assays: Spectral data matrix (rows: wavelengths, columns: samples)
        - ColData: DataFrame of sample metadata
        - RowData: DataFrame of wavelength information
    Step 6: Return the SummarizedExperiment object
End Function

\textbf{Example usage:}

\begin{quote}
\texttt{path <- "path/to/your/asd/files"} \\
\texttt{se <- read\_summarizedexperiment\_asd(path)} \\
\texttt{print(se)}
\end{quote}
